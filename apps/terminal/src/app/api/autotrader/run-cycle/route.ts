...
export async function POST(request: Request) {
  // ── Auth: accept either a valid session cookie OR an internal cron secret ──
  const cronSecret = process.env.CRON_SECRET;
  const internalCronHeader = request.headers.get('x-cron-secret');
  const isInternalCron = cronSecret &&
    cronSecret.length >= 32 &&
    internalCronHeader &&
    (() => {
      try {
        const a = Buffer.from(cronSecret);
        const b = Buffer.from(internalCronHeader);
  ...
  if (!lockAcquired) {
    ...
    return NextResponse.json(
      { success: false, reason: 'CYCLE_IN_FLIGHT', cycleId },
      { status: 409 }
    );
  }
  ...
  try {
    ...
    // lotUnits from config is used as the upper-bound cap in calculatePositionSize
    ...
    for (const symbol of activeInstruments) {
      ...
      // ── Step 4: Signal Generation & Volatility ATR Protection ───────────────
      ...
      // Risk-Derived Position Sizing (Task 1)
      ...
      // IDEMPOTENCY KEY (Task 3): deterministic from (accountId, symbol, cycleId).
      ...
      await insertGateDecision({
        ...
        accountState: {
          ...
          // Note: trailingStopDistance (when set) reflects approved protection distance
    ...
    // ── 9. breakEvenTriggerPips — check all open trades each cycle (Task 10) ─
    // Fetch current open trades from OANDA and move SL to entry if floating
    // profit exceeds breakEvenTriggerPips for any system-placed trade.
    const beResult = await checkBreakEven(adapter, accountId, config.riskProfile.breakEvenTriggerPips, apiKey, env, cycleId, executedLogs);
    void beResult; // errors are logged internally; never block the cycle
