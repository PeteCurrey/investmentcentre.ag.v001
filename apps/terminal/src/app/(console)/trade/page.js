...
const TradingViewChart_1 = __importDefault(require("../../../components/TradingViewChart"));
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const ratingColor = (r) => {
    if (r.includes('HIGH CONVICTION BUY'))
        return { bg: '#DCFCE7', text: '#166534', border: '#86EFAC' };
    if (r.includes('MODERATE BUY'))
        return { bg: '#D1FAE5', text: '#065F46', border: '#6EE7B7' };
    if (r.includes('HIGH CONVICTION SELL'))
        return { bg: '#FEE2E2', text: '#991B1B', border: '#FCA5A5' };
    if (r.includes('MODERATE SELL'))
        return { bg: '#FEE2E2', text: '#B91C1C', border: '#FCA5A5' };
    if (r.includes('CAUTION') || r.includes('AVOID'))
        return { bg: '#FEF3C7', text: '#92400E', border: '#FCD34D' };
    return { bg: '#F7F7F5', text: '#6B7280', border: '#E4E4DF' };
};
// ─── Component ───────────────────────────────────────────────────────────────
function TradePage() {
    const [inst, setInst] = (0, react_1.useState)(INSTRUMENTS[0]);
    const [timeframe, setTimeframe] = (0, react_1.useState)('15');
    const [direction, setDirection] = (0, react_1.useState)('BUY');
    const [units, setUnits] = (0, react_1.useState)('10000');
    const [stopLoss, setStopLoss] = (0, react_1.useState)('1.3050');
    const [takeProfit, setTakeProfit] = (0, react_1.useState)('1.3250');
    const [orderType, setOrderType] = (0, react_1.useState)('MARKET');
    const [limitPrice, setLimitPrice] = (0, react_1.useState)('');
    const [executing, setExecuting] = (0, react_1.useState)(false);
    const [execMsg, setExecMsg] = (0, react_1.useState)(null);
    const [aiLoading, setAiLoading] = (0, react_1.useState)(false);
    const [aiAnalysis, setAiAnalysis] = (0, react_1.useState)({
    ...
    const [aiError, setAiError] = (0, react_1.useState)(null);
    ...
    // ── Handlers ──
    const handleSelectInstrument = (0, react_1.useCallback)((i) => {
        setInst(i);
        setExecMsg(null);
    }, []);
    ...
    const handleAnalyse = (0, react_1.useCallback)(async () => {
        setAiLoading(true);
        setAiError(null);
        try {
    ...
    // ── Render ──
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '20px', fontFamily: '"DM Mono", monospace', color: '#14181B' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {INSTRUMENTS.map(i => {
            ...
            return (<button key={i.symbol} onClick={() => handleSelectInstrument(i)} style={{
                    padding: '6px 12px',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          ...
          {TIMEFRAMES.map(tf => (<button key={tf.value} onClick={() => setTimeframe(tf.value)} style={{
                padding: '2px 7px',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '16px', alignItems: 'start' }}>
        ...
        <div style={{ border: '1px solid #E4E4DF', backgroundColor: '#0A0D12', display: 'flex', flexDirection: 'column' }}>
          ...
          <TradingViewChart_1.default symbol={inst.tvSymbol} interval={timeframe} theme="dark" height={560} showSidebar/>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
            {['MARKET', 'LIMIT'].map(ot => (<button key={ot} onClick={() => setOrderType(ot)} style={{
                padding: '6px',
          ...
          <div style={{ border: '1px solid #E4E4DF', backgroundColor: '#FFFFFF', padding: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '10px' }}>
              <div>
                ...
                <input type="text" value={units} onChange={(e) => setUnits(e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid #E4E4DF', fontFamily: '"DM Mono", monospace', fontSize: '11px', boxSizing: 'border-box', color: '#14181B' }}/>
              </div>

              {orderType === 'LIMIT' && (<div>
                  <label style={{ color: '#6B7280', display: 'block', marginBottom: '3px' }}>LIMIT PRICE</label>
                  <input type="text" value={limitPrice} onChange={(e) => setLimitPrice(e.target.value)} placeholder={inst.price} style={{ width: '100%', padding: '7px 8px', border: '1px solid #1C3A5E', fontFamily: '"DM Mono", monospace', fontSize: '11px', boxSizing: 'border-box', color: '#14181B' }}/>
                </div>)}

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                <div>
                  <label style={{ color: '#6B7280', display: 'block', marginBottom: '3px' }}>STOP LOSS</label>
                  <input type="text" value={stopLoss} onChange={(e) => setStopLoss(e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid #FCA5A5', fontFamily: '"DM Mono", monospace', fontSize: '11px', boxSizing: 'border-box', color: '#991B1B' }}/>
                </div>
                <div>
                  <label style={{ color: '#6B7280', display: 'block', marginBottom: '3px' }}>TAKE PROFIT</label>
                  <input type="text" value={takeProfit} onChange={(e) => setTakeProfit(e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid #86EFAC', fontFamily: '"DM Mono", monospace', fontSize: '11px', boxSizing: 'border-box', color: '#166534' }}/>
                </div>
              </div>
            </div>
          ...
          <div style={{ border: '1px solid #1E293B', backgroundColor: '#0F172A', color: '#F8FAFC', padding: '14px', flex: 1 }}>
            ...
            {aiAnalysis && aColor && (<div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '10px' }}>
                {/* Rating Badge */}
            ...
            {!aiAnalysis && !aiLoading && (<div style={{ fontSize: '10px', color: '#475569', textAlign: 'center', padding: '16px 0' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', fontFamily: '"DM Mono", monospace' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #E4E4DF', textAlign: 'left', color: '#6B7280' }}>
              {['TIMESTAMP', 'SOURCE', 'INSTRUMENT', 'DIRECTION', 'UNITS', 'FILL PRICE', 'ORDER ID', 'STATUS'].map(h => (<th key={h} style={{ padding: '8px 14px', fontWeight: 500 }}>{h}</th>))}
            </tr>
          </thead>
          <tbody>
            {logs.map(log => (<tr key={log.id} style={{ borderBottom: '1px solid #F0F0EC' }}>
