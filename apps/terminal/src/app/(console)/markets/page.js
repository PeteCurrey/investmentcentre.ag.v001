...
function MarketsPage() {
    const prices = [
        { id: 'mkt_gbpusd', symbol: 'GBP/USD', name: 'British Pound / US Dollar', price: '1.3145', change: '+0.42%', source: 'twelve_data', age: 5, bias: 'NEUTRAL', note: 'Range-bound 1.295–1.328. FOMC catalyst.' },
        { id: 'mkt_eurusd', symbol: 'EUR/USD', name: 'Euro / US Dollar', price: '1.0920', change: '-0.15%', source: 'twelve_data', age: 5, bias: 'BEARISH', note: 'USD strength on delayed cut cycle.' },
        { id: 'mkt_wti', symbol: 'WTI_CRUDE', name: 'WTI Light Sweet Crude', price: '$78.40', change: '+1.85%', source: 'twelve_data', age: 10, bias: 'BULLISH', note: 'EIA draw -3.4M bbl vs +1.2M consensus.' },
        { id: 'mkt_spx', symbol: 'SPX_INDEX', name: 'S&P 500 Index', price: '5,520.40', change: '+0.28%', source: 'finnhub', age: 15, bias: 'NEUTRAL', note: 'Earnings season support. Fed rate sensitivity.' },
    ];
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ border: '1px solid #E4E4DF' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)' }}>
          {prices.map((p, idx) => {
            ...
            return (<link_1.default key={p.symbol} href={`/story/${p.id}`} style={{ textDecoration: 'none' }}>
                <div style={{
                    padding: '16px',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <Value_1.Value provenance={{ value: p.price, source: p.source, sourceTimestamp: new Date().toISOString(), stalenessSeconds: p.age }}/>
                      <div style={{
                    fontSize: '12px',
                    fontFamily: '"DM Mono", monospace',
                    fontWeight: 700,
      ...
      <div style={{ border: '1px solid #E4E4DF' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {shortPositions.map((s, idx) => (<link_1.default key={s.id} href={`/story/${s.id}`} style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{
                padding: '14px 16px',
                ...
                cursor: 'pointer',
                transition: 'background-color 0.15s ease',
            }} onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#FEE2E2')} onMouseLeave={e => (e.currentTarget.style.backgroundColor = '#FFFFFF')}>
