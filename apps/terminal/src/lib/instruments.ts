...
export interface Instrument {
  symbol: string;          // Display: "GBP/USD", "SPX500"
  oandaId: string;         // OANDA v20: "GBP_USD", "SPX500_USD"
...
export function getInstrument(symbolOrOandaId: string): Instrument | undefined {
  if (!symbolOrOandaId) return undefined;
  const s = symbolOrOandaId.trim();
  // 1. Direct match by symbol
  let found = INSTRUMENT_UNIVERSE.find(i => i.symbol === s);
  if (found) return found;
  // 2. Direct match by oandaId
  found = INSTRUMENT_UNIVERSE.find(i => i.oandaId === s);
  if (found) return found;
  // 3. Fallback normalized match (e.g. "SPX 500" -> "SPX500", "GBP_USD" -> "GBP/USD")
  const norm = s.replace(/[\/\s_-]/g, '').toUpperCase();
  return INSTRUMENT_UNIVERSE.find(i => 
    i.symbol.replace(/[\/\s_-]/g, '').toUpperCase() === norm ||
    i.oandaId.replace(/[\/\s_-]/g, '').toUpperCase() === norm
  );
}

export function getOandaId(symbolOrOandaId: string): string | undefined {
  const inst = getInstrument(symbolOrOandaId);
  if (inst) return inst.oandaId;
  return symbolOrOandaId.includes('/') ? symbolOrOandaId.replace('/', '_') : symbolOrOandaId;
}

export function getDisplaySymbol(symbolOrOandaId: string): string {
  const inst = getInstrument(symbolOrOandaId);
