...
exports.clearCouncilCache = clearCouncilCache;
const core_1 = require("@meridian/core");
const crypto_1 = __importDefault(require("crypto"));
const log = (0, core_1.createLogger)('Council');
...
function parseSnapshotToObservation(o, idx) {
    return {
        ...
        licence_class: o.licence_class || 'INTERNAL_ONLY',
...
class CouncilOrchestrator {
    ...
    async evaluate(input) {
        ...
        if (consensusCache.has(inputHash)) {
            ...
            return (0, core_1.ok)({ ...cachedResult, cached: true });
        }
        const opinions = [];
        let totalTokenCostUsd = 0;
        const validObsIds = new Set(input.observationsSnapshot.map((o, idx) => String(o.id || `obs_${idx}`)));
        const promptContext = `
Instrument: ${input.instrument}
...
`;
        ...
        const realObservations = input.observationsSnapshot.map(parseSnapshotToObservation);
