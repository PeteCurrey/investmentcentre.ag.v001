...
class DeltaEngine {
     ...
     */
    detectContradictions(observations, divergenceThresholdPct = 2.0) {
        ...
        for (const [key, obsGroup] of grouped.entries()) {
            ...
            for (let i = 0; i < obsGroup.length; i++) {
                for (let j = i + 1; j < obsGroup.length; j++) {
                    ...
                    if (obsA.source_id === obsB.source_id)
                        continue;
     ...
     */
    detectChanges(pastObs, newObs) {
        if (pastObs.metric !== newObs.metric)
            return null;
