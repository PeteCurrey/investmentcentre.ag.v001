...
const envPath = (0, path_1.resolve)(process.cwd(), '.env.local');
try {
    ...
    console.warn('[env] No .env.local found');
}
const index_1 = require("../packages/council/src/index");
async function main() {
    ...
    console.log(`Obs IDs: ${observations.map((o) => o.id).join(', ')}`);
    console.log('══════════════════════════════════════════════════════════\n');
    const orchestrator = new index_1.CouncilOrchestrator();
    ...
    for (const op of c.opinions) {
        const valid = op.citations.filter((x) => validIds.has(x));
        const invalid = op.citations.filter((x) => !validIds.has(x));
...
//# sourceMappingURL=run_council.js.map