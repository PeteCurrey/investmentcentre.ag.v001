...
# 1. Helpers
if 'function getTvSymbol' not in content:
...
'''
    target = 'function ratingColor'
    content = content.replace(target, helpers + target, 1)

# 2. States
states_target = "const [pnlTimeframe, setPnlTimeframe]"
if 'const [bulkModalOpen, setBulkModalOpen]' not in content:
    new_states = '''// Bulk Actions State
  ...
  ''' + states_target
    content = content.replace(states_target, new_states, 1)

# 3. Countdown effect
if 'setRefreshCountdown(prev' not in content:
    cd_target = "const inst = instruments.find"
    cd_code = '''useEffect(() => {
  ...
  ''' + cd_target
    content = content.replace(cd_target, cd_code, 1)

# 4. handleBulkClose
if 'const handleBulkClose =' not in content:
    bc_target = "const fetchOandaData = useCallback"
    bc_code = '''const handleBulkClose = async (positionsToClose: Position[]) => {
  ...
  ''' + bc_target
    content = content.replace(bc_target, bc_code, 1)

# 5. Open Chart in ticker strip
if 'OPEN CHART' not in content:
    ts_target = "{TIMEFRAMES.map(tf => ("
    ts_code = '''<button onClick={() => setChartModalInstrument({ symbol: inst.symbol, tvSymbol: inst.tvSymbol })} style={{
          ...
          ''' + ts_target
    content = content.replace(ts_target, ts_code, 1)

# 6. Mode Clarity Banner
if 'MODE CLARITY BANNER' not in content:
    mc_target = "{/* ── Closed Trades P&L Performance Analytics ── */}"
    mc_code = '''{/* ── MODE CLARITY BANNER ── */}
      <div style={{
        padding: '10px 16px',
        marginBottom: '16px',
      ...
      ''' + mc_target
    content = content.replace(mc_target, mc_code, 1)

# 7. Positions Header
if 'BULK ACTIONS' not in content:
    hdr_old = '''<span style={{ padding: '1px 7px', backgroundColor: positions.length > 0 ? '#22C55E' : '#475569', color: '#FFFFFF', fontSize: '9px', fontWeight: 800 }}>{positions.length}</span>
          </div>
    ...
    hdr_new = '''<span style={{ padding: '1px 7px', backgroundColor: positions.length > 0 ? '#22C55E' : '#475569', color: '#FFFFFF', fontSize: '9px', fontWeight: 800 }}>{positions.length}</span>
            {positions.length > 0 && (() => {
    ...
    content = content.replace(hdr_old, hdr_new, 1)

# 8. Table Headers
if "'CHART'" not in content:
    content = content.replace(
        "['INSTRUMENT', 'DIRECTION', 'UNITS', 'ENTRY', 'UNREALIZED P&L', 'OPENED AT', 'DETAILS', 'CLOSE']",
        "['INSTRUMENT', 'CHART', 'DIRECTION', 'UNITS', 'ENTRY', 'UNREALIZED P&L', 'OPENED / TIME HELD', 'DETAILS', 'CLOSE']",
        1
    )

# 9. Table Rows
row_old = '''<tr style={{ borderBottom: '1px solid #F0F0EC', backgroundColor: expandedPos === p.id ? '#F0F9FF' : (p.pnlPositive ? '#F0FDF4' : '#FFF5F5') }}>
...
# 10. Modals
modals_code = '''{/* ── BULK ACTIONS MODAL ── */}
      {bulkModalOpen && (
        <div
...
style_target = "<style>{`"
if '/* ── BULK ACTIONS MODAL ── */' not in content:
    content = content.replace(style_target, modals_code + style_target, 1)
...
print("Python patch script executed successfully")
